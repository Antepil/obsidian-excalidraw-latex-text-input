...
const latexSuiteSnippets: LatexSnippet[] = [
  ...
  { trigger: "tayl", replacement: "${0:f}(${1:x} + ${2:h}) = ${0:f}(${1:x}) + ${0:f}'(${1:x})${2:h} + ${0:f}''(${1:x}) \\frac{${2:h}^{2}}{2!} + \\dots$3", options: "mA" },
  {
    ...
    replacement: (match) => {
      ...
      const rows = Array.from({ length: size }, (_, row) => {
        return Array.from({ length: size }, (_, col) => (row === col ? "1" : "0")).join(" & ");
      });
  ...
  },
  {
    ...
    replacement: (match) => {
      ...
      const whitespace = groups.whitespace ?? "";
...
export default latexSuiteSnippets;
